...
def perform_string_manipulations(spark):
    ...
    pandas_df['full_info'] = pandas_df['patient_name'] + ' - ' + pandas_df['diagnosis_lower']

    # PySpark DataFrame creation directly from data
    spark_df = spark.createDataFrame(data, schema=columns)
    spark_df = spark_df.withColumn("diagnosis_lower", lower(col("diagnosis")))
    spark_df = spark_df.withColumn("full_info", concat_ws(" - ", col("patient_name"), col("diagnosis_lower")))
